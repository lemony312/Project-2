from nd_app.models import db, Restaurants, Favorites, Customers 
...
def show_fav_restaurants(customer_id, area=None, rank=None, search_key=None):
	query = Restaurants.query.filter(Restaurants.is_deleted == 'N')
	...
	# work on making a join table in the future
	fav_query = Favorites.query.filter(Favorites.customers_id == customer_id).all()
	ids = map(lambda x: int(x.restaurant_id), fav_query) 
	querys=[]
	for i in ids:
		querys.append(query.filter_by(id = i).first())

	return querys

def is_favorite(customer_id, restaurant_id):
	print customer_id, restaurant_id
	query = Favorites.query.filter(Favorites.customers_id == customer_id, Favorites.restaurant_id == restaurant_id).first()
