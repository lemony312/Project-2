...
function initialise_document_ready_functions()
{
    ...
    var amount_of_pixels_as_buffer_between_sections = 1 * ($(window).height()); // used in update_active_sections_on_scroll();
    ...
    $("#description-image-slider").owlCarousel({
            loop:true,
            autoplay: true,
            lazyLoad: true,
            autoplayTimeout: 3000,
    ...
    { 
        ...
        amount_of_pixels_as_buffer_between_sections = 1 * ($(window).height()); // used in update_active_sections_on_scroll();
...
function initialise_window_load_functions()
{
    ...
    populate_and_open_modal(event, 'show_popup');
